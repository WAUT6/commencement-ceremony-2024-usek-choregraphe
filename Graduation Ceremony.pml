<?xml version="1.0" encoding="UTF-8" ?>
<Package name="Graduation Ceremony" format_version="4">
    <Manifest src="manifest.xml" />
    <BehaviorDescriptions>
        <BehaviorDescription name="behavior" src="feedback_behavior" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="menu_behavior" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="selfie_raw_behavior" xar="behavior.xar" />
        <BehaviorDescription name="behavior" src="selfie_looped" xar="behavior.xar" />
    </BehaviorDescriptions>
    <Dialogs>
        <Dialog name="confirmation_dialog" src="confirmation_dialog/confirmation_dialog.dlg" />
        <Dialog name="ExampleDialog" src="menu_behavior/ExampleDialog/ExampleDialog.dlg" />
        <Dialog name="main_menu_dialog" src="main_menu_dialog/main_menu_dialog.dlg" />
        <Dialog name="picture_dialog" src="picture_dialog/picture_dialog.dlg" />
    </Dialogs>
    <Resources>
        <File name="all" src="html/css/all.css" />
        <File name="displayinfo" src="html/css/displayinfo.css" />
        <File name="formatting" src="html/css/formatting.css" />
        <File name="home" src="html/css/home.css" />
        <File name="rating" src="html/css/rating.css" />
        <File name="w3" src="html/css/w3.css" />
        <File name="index" src="html/index.html" />
        <File name="all" src="html/js/all.js" />
        <File name="displayimage" src="html/js/displayimage.js" />
        <File name="displayinfo" src="html/js/displayinfo.js" />
        <File name="fastclick" src="html/js/fastclick.js" />
        <File name="help" src="html/js/help.js" />
        <File name="index" src="html/js/index.js" />
        <File name="jquery" src="html/js/jquery.js" />
        <File name="qievents" src="html/js/qievents.js" />
        <File name="vars" src="html/js/vars.js" />
        <File name="confirmation" src="html/pages/confirmation.html" />
        <File name="displayimage" src="html/pages/displayimage.html" />
        <File name="displayinfo" src="html/pages/displayinfo.html" />
        <File name="displaytext" src="html/pages/displaytext.html" />
        <File name="getinput" src="html/pages/getinput.html" />
        <File name="help" src="html/pages/help.html" />
        <File name="noPage" src="html/pages/noPage.html" />
        <File name="rating" src="html/pages/rating.html" />
        <File name="blank" src="html/pics/blank.jpg" />
        <File name="feedback" src="html/pics/feedback.jpg" />
        <File name="pepper-question" src="html/pics/pepper-question.png" />
        <File name="pepper-standing" src="html/pics/pepper-standing.png" />
        <File name="ques1" src="html/pics/ques1.jpg" />
        <File name="ques2" src="html/pics/ques2.png" />
        <File name="ques3" src="html/pics/ques3.jpg" />
        <File name="ques4" src="html/pics/ques4.jpg" />
        <File name="thankYou" src="html/pics/thankYou.jpg" />
        <File name="useklogoo" src="html/pics/useklogoo.jpg" />
        <File name="useklogooo" src="html/pics/useklogooo.jpg" />
        <File name="welcome" src="html/pics/welcome.jpg" />
        <File name="fa-brands-400" src="html/webfonts/fa-brands-400.eot" />
        <File name="fa-brands-400" src="html/webfonts/fa-brands-400.svg" />
        <File name="fa-brands-400" src="html/webfonts/fa-brands-400.ttf" />
        <File name="fa-brands-400" src="html/webfonts/fa-brands-400.woff" />
        <File name="fa-brands-400" src="html/webfonts/fa-brands-400.woff2" />
        <File name="fa-regular-400" src="html/webfonts/fa-regular-400.eot" />
        <File name="fa-regular-400" src="html/webfonts/fa-regular-400.svg" />
        <File name="fa-regular-400" src="html/webfonts/fa-regular-400.ttf" />
        <File name="fa-regular-400" src="html/webfonts/fa-regular-400.woff" />
        <File name="fa-regular-400" src="html/webfonts/fa-regular-400.woff2" />
        <File name="fa-solid-900" src="html/webfonts/fa-solid-900.eot" />
        <File name="fa-solid-900" src="html/webfonts/fa-solid-900.svg" />
        <File name="fa-solid-900" src="html/webfonts/fa-solid-900.ttf" />
        <File name="fa-solid-900" src="html/webfonts/fa-solid-900.woff" />
        <File name="fa-solid-900" src="html/webfonts/fa-solid-900.woff2" />
        <File name="bg_animation" src="html/css/bg_animation.css" />
        <File name="div_animation_move" src="html/css/div_animation_move.css" />
        <File name="part_animation" src="html/css/part_animation.css" />
        <File name="background" src="html/js/background.js" />
        <File name="part_bg" src="html/js/part_bg.js" />
        <File name="usek_logo_2" src="html/pics/usek_logo_2.jpg" />
        <File name="usek_logo_3" src="html/pics/usek_logo_3.jpg" />
        <File name="usek_logo_4" src="html/pics/usek_logo_4.jpg" />
        <File name="main_menu" src="html/css/main_menu.css" />
        <File name="main_menu" src="html/pages/main_menu.html" />
        <File name="grad-gown" src="html/pics/grad-gown.jpg" />
        <File name="selfie_styles" src="html/css/selfie_styles.css" />
        <File name="take_picture" src="html/pages/take_picture.html" />
        <File name="pepper-selfie-pic" src="html/pics/pepper-selfie-pic.jpg" />
        <File name="picture_now" src="html/pages/picture_now.html" />
        <File name="enjoy" src="html/pages/enjoy.html" />
        <File name="LICENSE" src="LICENSE" />
        <File name="logo-99" src="html/pics/logo-99.jpg" />
        <File name="logo-hurricane" src="html/pics/logo-hurricane.jpg" />
        <File name="usek-logo-resized" src="html/pics/usek-logo-resized.jpg" />
    </Resources>
    <Topics>
        <Topic name="confirmation_dialog_enu" src="confirmation_dialog/confirmation_dialog_enu.top" topicName="confirmation_dialog" language="en_US" />
        <Topic name="concepts" src="confirmation_dialog/concepts.top" topicName="concepts" language="en_US" />
        <Topic name="lexicon_enu" src="confirmation_dialog/lexicon_enu.top" topicName="lexicon" language="en_US" />
        <Topic name="ExampleDialog_enu" src="menu_behavior/ExampleDialog/ExampleDialog_enu.top" topicName="ExampleDialog" language="en_US" />
        <Topic name="main_menu_dialog_enu" src="main_menu_dialog/main_menu_dialog_enu.top" topicName="main_menu_dialog" language="en_US" />
        <Topic name="picture_dialog_enu" src="picture_dialog/picture_dialog_enu.top" topicName="picture_dialog" language="en_US" />
    </Topics>
    <IgnoredPaths />
    <Translations auto-fill="en_US">
        <Translation name="translation_en_US" src="translations/translation_en_US.ts" language="en_US" />
    </Translations>
</Package>
